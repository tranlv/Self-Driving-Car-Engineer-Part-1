...
"""Model design"""
def Lenet_5_second_attempt(input_image):    
    ...
    conv2 = convolution_layer(conv2, 5, 10, 20, 'VALID')
    ...
    conv4 = tf.nn.relu(conv2)
    # max_pool: output = 8x8x40
    conv4 = max_spooling(conv2, 'VALID')
    # drop-out
    ...
    logits = fully_connected_layer(fc2, 80, 43)   
    
    return logits

def evaluate_second_attempt(X_data, y_data, keep_prob):
...
if __name__ == "__main__":
  ...
  with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        ...
        valid_loss, valid_accuracy = evaluate(X_valid_final_1, y_valid_original, 1.0)
        valid_loss_history.append(valid_loss)

        train_loss, train_accuracy = evaluate(X_train_final_1, y_train_original, 1.0)
